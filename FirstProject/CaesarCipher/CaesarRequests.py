...
def first_request(cipher, input_file, key, output_file): # encrypt
    """
        ...
        Example usage:\n
        >>> cipher = CaesarCipher()
        >>> first_request(cipher, 'plaintext.txt', 'ROT13', 'encrypted.txt')
        # This will encrypt the content of 'plaintext.txt' using a ROT13 key
...
def second_request(cipher, input_file, key, output_file):
    """
            Processes the initial request for deryption. It reads the input file, decrypts the content using the provided key,
            ...
            >>> cipher = CaesarCipher()
            >>> first_request(cipher, 'plaintext.txt', 'ROT13', 'encrypted.txt')
            # This will decrypt the content of 'plaintext.txt' using a ROT13 key
            # and save the decrypted text to 'encrypted.txt'.
            """
    if not os.path.isfile(input_file):
        print("Input file does not exist!")
        return
...
def third_request(cipher, input_file, output_file):
    """
        ...
        Example usage:\n
        >>> cipher = CaesarCipher()
        >>> third_request(cipher, 'encrypted.txt', 'decrypted.txt')
        # This will attempt to break the encryption of 'encrypted.txt' and
        # save the decrypted text to 'decrypted.txt', with the key saved to 'key.txt'.
        """
    if not os.path.isfile(input_file):
        print("Input file does not exist!")
        return
