...
from CaesarCipher.CaesarRequests import *

def caesar_menu(request, input_file, key_txt, output_file):
    """
    ...
    if request in ['de', 'en']: # if request is encrypt or decrypt
        ...
        if request == 'en': # encrypt
            first_request(cipher, input_file, key, output_file)
        elif request == 'de': # decrypt
            second_request(cipher, input_file, key, output_file)
    elif request == 'br':
        if not os.path.isfile(key_txt):
            print("Key file does not exist!")
            return
        cipher = Caesar()
        third_request(cipher, input_file, output_file)
    else:
